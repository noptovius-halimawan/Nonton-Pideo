...
public class MovieFragment extends Fragment {
    ...
    private void showViewBasedOnMode(String mode, String query) {
        ...
        if (Objects.requireNonNull(mode).equals(MainActivity.HOME_MODE)) {
            ...
            if(query.equals(MainActivity.EMPTY_SEARCH)){
                videoViewModel.getAndInsertUpcomingMoviesToLiveData();
            }else{
                videoViewModel.searchAndInsertMovieToLiveData(query);
            }
        } else {
            videoViewModel = initializeFavoriteMoviesViewModel();
            videoViewModel.getAndInsertFavoriteMoviesToLiveData(Objects.requireNonNull(getActivity()).getApplicationContext());
        }
    }

    private void initializeViews() {
        rv = Objects.requireNonNull(getView()).findViewById(R.id.rv_movie);
    ...
    private VideoViewModel initializeViewModel() {
        VideoViewModel videoViewModel = ViewModelProviders.of(this)
                .get("MovieViewModel", VideoViewModel.class);
        videoViewModel.getUpcomingMoviesLiveData().observe(this, videoArrayListObserver);
    ...
    private VideoViewModel initializeFavoriteMoviesViewModel() {
        VideoViewModel videoViewModel = ViewModelProviders.of(this)
                .get("MovieViewModel", VideoViewModel.class);
        videoViewModel.getUpcomingMoviesLiveData().observe(this, videoArrayListObserver);
        return videoViewModel;
    ...
    private final Observer<ArrayList<Video>> videoArrayListObserver = new Observer<ArrayList<Video>>() {
        @Override
